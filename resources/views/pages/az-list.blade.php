                    ...
                    <div class="col-xs-8 hidden-xs">
                        <nav aria-label="breadcrumbs" class="rank-math-breadcrumb">
                            <p><a href="{{ url('/') }}">Trang chủ</a><span class="separator"> » </span><span
                                    class="last">Danh sách phim từ A đến Z</span></p>
                        </nav>
                    </div>
            ...
            <section>
                <div class="section-bar clearfix">
                    <h3 class="section-title">
                        <span>Danh sách phim kí tự {{ $az }}</span>
                    </h3>
                </div>
                <div class="halim_box">
                    @if (!$listMovieBySearch->count())
                        <h5>Oops...Không tìm thấy - - trên hệ thống</h5>
                    @else
                        @foreach ($listMovieBySearch as $movie)
                            <article class="col-md-3 col-sm-3 col-xs-6 thumb grid-item post-{{ $movie->id }}">
...
@endforeach
                                            ...
                                            data-original-title="<span class=film-title>{{ $movie->title }}</span>">
                                        </div>
                ...
                <div class="clearfix"></div>
                <div class="text-center">
                    {{ $listMovieBySearch->links('vendor.pagination.custom') }}
                </div>
            </section>
        </main>
