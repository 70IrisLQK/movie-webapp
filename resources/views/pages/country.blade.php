                    ...
                    <div class="col-xs-8 hidden-xs">
                        <nav aria-label="breadcrumbs" class="rank-math-breadcrumb">
                            <p><a href="{{ url('/') }}">Trang chủ</a><span class="separator"> » </span><span
                                    class="last">{{ $listCountryBySlug->title }}</span></p>
                        </nav>
                    </div>
            ...
            <section>
                <div class="section-bar clearfix">
                    <h3 class="section-title">
                        <span>Danh sách phim {{ $listCountryBySlug->name }}</span>
                    </h3>
                </div>
                <div class="halim_box">
                    @if (!$listMovieByCountry->count())
                        <h5>Oops...Không tìm thấy - - trên hệ thống</h5>
                    @else
                        @foreach ($listMovieByCountry as $movie)
                                ...
                                <div class="halim-item">
                                    <a class="halim-thumb" href="{{ route('phim', [$movie->slug]) }}"
                                        ...
                                        <figure><img class="blur-up img-responsive lazyautosizes lazyloaded"
                                                data-sizes="auto" data-src="{{ asset('uploads/movie/' . $movie->image) }}"
                                                alt="{{ $movie->name }}" title="{{ $movie->name }}" sizes="149px"
                                        ...
                                        <div class="icon_overlay" data-html="true" data-toggle="halim-popover"
                                            ...
                                            title="<span class=film-title>{{ $movie->name }}</span>"
...
{{ $description }}
  ...
  <p class=category>Quốc gia: <span class=category-name>  @foreach ($movie->countries as $country)
...
@endforeach
                ...
                <div class="clearfix"></div>
                <div class="text-center">
                    {{ $listMovieByCountry->links('vendor.pagination.custom') }}
                </div>
            </section>
        </main>
